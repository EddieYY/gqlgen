...
type Resolvers interface {
	Mutation_addMessage(ctx context.Context, text string, userId int, chatroomId int) (Message, error)
	Query_chatrooms(ctx context.Context) ([]Chatroom, error)
	Query_chatroom(ctx context.Context, id int) (Chatroom, error)
	Query_messages(ctx context.Context, chatroomId *int) ([]Message, error)
	Query_users(ctx context.Context, chatroomId *int) ([]User, error)
	Query_user(ctx context.Context, id *int, search *string) (*User, error)

	Subscription_messageAdded(ctx context.Context, chatroomId int) (chan Message, error)
}
...
func NewExecutor(resolvers Resolvers) func(context.Context, string, string, map[string]interface{}, io.Writer) []*errors.QueryError {
	return func(ctx context.Context, document string, operationName string, variables map[string]interface{}, w io.Writer) []*errors.QueryError {
		...
		if op.Type == query.Query {
			...
			data = c._subscription(op.Selections, nil)
		} else {
...
func (ec *executionContext) _chatroom(sel []query.Selection, it *Chatroom) jsonw.Writer {
	fields := ec.collectFields(sel, chatroomImplementors, map[string]bool{})
...
// nolint: gocyclo, errcheck, gas, goconst
func (ec *executionContext) _message(sel []query.Selection, it *Message) jsonw.Writer {
	fields := ec.collectFields(sel, messageImplementors, map[string]bool{})
	out := jsonw.NewOrderedMap(len(fields))
	for i, field := range fields {
		out.Keys[i] = field.Alias
		out.Values[i] = jsonw.Null
...
func (ec *executionContext) _mutation(sel []query.Selection, it *interface{}) jsonw.Writer {
	...
	for i, field := range fields {
		...
		case "addMessage":
			...
			var arg2 int
			if tmp, ok := field.Args["chatroomId"]; ok {
				tmp2, err := coerceInt(tmp)
				if err != nil {
					ec.Error(err)
					continue
			...
			res, err := ec.resolvers.Mutation_addMessage(ec.ctx, arg0, arg1, arg2)
			if err != nil {
				ec.Error(err)
				continue
			}
	...
	for i, field := range fields {
		...
		case "chatrooms":
			...
			go func(i int, field collectedField) {
				defer ec.wg.Done()
				res, err := ec.resolvers.Query_chatrooms(ec.ctx)
				if err != nil {
					ec.Error(err)
					return
				}
				...
				for idx1 := range res {
					var tmp1 jsonw.Writer
					tmp1 = ec._chatroom(field.Selections, &res[idx1])
					arr1 = append(arr1, tmp1)
				}
				out.Values[i] = arr1
			}(i, field)
		case "chatroom":
			...
			go func(i int, field collectedField) {
				defer ec.wg.Done()
				res, err := ec.resolvers.Query_chatroom(ec.ctx, arg0)
				if err != nil {
					ec.Error(err)
					return
				}
		...
		case "messages":
			...
			go func(i int, field collectedField) {
				defer ec.wg.Done()
				res, err := ec.resolvers.Query_messages(ec.ctx, arg0)
				if err != nil {
					ec.Error(err)
					return
				}
		...
		case "users":
			...
			go func(i int, field collectedField) {
				defer ec.wg.Done()
				res, err := ec.resolvers.Query_users(ec.ctx, arg0)
				if err != nil {
					ec.Error(err)
					return
				}
		...
		case "user":
			...
			var arg1 *string
			if tmp, ok := field.Args["search"]; ok {
				tmp2, err := coerceString(tmp)
				if err != nil {
					ec.Error(err)
					continue
			...
			go func(i int, field collectedField) {
				defer ec.wg.Done()
				res, err := ec.resolvers.Query_user(ec.ctx, arg0, arg1)
				if err != nil {
					ec.Error(err)
					return
				}
...
// nolint: gocyclo, errcheck, gas, goconst
func (ec *executionContext) _subscription(sel []query.Selection, it *interface{}) jsonw.Writer {
	fields := ec.collectFields(sel, subscriptionImplementors, map[string]bool{})
	out := jsonw.NewOrderedMap(len(fields))
	for i, field := range fields {
		out.Keys[i] = field.Alias
		out.Values[i] = jsonw.Null
		...
		case "messageAdded":
			...
			go func(i int, field collectedField) {
				defer ec.wg.Done()
				res, err := ec.resolvers.Subscription_messageAdded(ec.ctx, arg0)
				if err != nil {
					ec.Error(err)
					return
				}
